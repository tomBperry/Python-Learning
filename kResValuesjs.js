...
E48 = [1.00, 1.05, 1.10, 1.15, 1.21, 1.27, 1.33, 1.40, 1.47, 1.54, 1.62, 1.69, 1.78, 1.87, 1.96, 2.05, 2.15,
...
E96 = [1.00, 1.02, 1.05, 1.07, 1.10, 1.13, 1.15, 1.18, 1.21, 1.24, 1.27, 1.30, 1.33, 1.37, 1.40, 1.43, 1.47,
...
E192 = [1.00, 1.01, 1.02, 1.04, 1.05, 1.06, 1.07, 1.09, 1.10, 1.11, 1.13, 1.14, 1.15, 1.17, 1.18, 1.20, 1.21,
...
const decimals = 11;
const K = round(0.9611946667, decimals);
const acc = 0.00001;
const order = 10;

let info = findPairs(E192 );
info = simmer(info);

if (info.length != 0) {
  for (i = info.length - 1; i >= 0; i--) {
    console.log("Pair: " + (i + 1).toString() + ", r1 = " +
      info[i][1].toString() + ", r2 = " + info[i][2].toString() +
      ", Difference in accuracy  = " + info[i][0].toString() +
      ", k = " + info[i][3].toString())
  }

  console.log("There are " + info.length.toString() + " possible pairs")

} else {
  console.log("There are no possible pairs.")
}


function findPairs(r) {
  ...
  for (i = 0; i < r.length; i++) {
    for (j = 0; j < r.length; j++) {
      for (n = 1; n < order; n++) { // Check what order actually equals
        for (m = 1; m < order; m++) {
          ...
          k = r2 / (r1 + r2);
          k = round(k, decimals);
          if (Math.abs(K - k) < acc / 2) {
            ls.push([round(Math.abs(K - k), decimals), round(r1, 3), round(r2, 3), k])
          }
        }
      }
    }
...
function simmer(arr) {
  ...
  toDel = uniq_fast(toDel)
  // console.log(toDel)
  // console.log(arr.length)

  for (i = toDel.length - 1; i > -1; i--) {
