...
K = 0.9611946667  # The ratio of desired voltage to input voltage # 0.9611946667
acc = 0.00001  # to what accuracy you need this desired voltage
...
def find_pairs(r):
        ...
        for j in range(len(r)):
            for m in range(10):
                for n in range(10):
                    ...
                    if abs(K - k) < acc / 2:
                        ls.append([round(abs(K-k), decimals), round(k, decimals), r1, r2])

    for n in range(len(ls)):
        for j in range(n, len(ls)):
            if n != j:
                if ls[n][1] == ls[j][1]:
                    to_del.append(j)
                    break
    ...
    for n in range(len(to_del) - 1, -1, -1):
        del ls[to_del[n]]
    ls.sort()
...
if len(ls):
    for i in range(len(ls) - 1, -1, -1):
        print("Pair: " + str(i + 1) + ", r1 = " + str(ls[i][2]) + ", r2 = " + str(ls[i][3])
              + ", Difference in accuracy  = " + str(ls[i][0]) + ", k = " + str(ls[i][1]))

    print("There are " + str(len(ls)) + " possible pairs")
else:
    print("There are no possible pairs.")
